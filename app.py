...
async def lifespan(app: FastAPI):
    # Startup: carrega modelo na inicialização
    print("Inicializando aplicação IA...")
    try:
        get_model()
        print(f"✓ Modelo carregado: {_model_path}")
...
def get_model():
    ...
    if _model is None:
        ...
        if env_path:
            ...
            # Padrão: usar models/best.pt (modelo customizado treinado)
            chosen = "models/best.pt"
            # Se não existir, seria necessário treinar ou usar um modelo pré-treinado
            if not Path(chosen).exists():
                print(f"Aviso: Modelo padrão {chosen} não encontrado")
                # Fallback para YOLOv8 nano pré-treinado se necessário
                # chosen = "yolov8n.pt"
