...
if [ "$IMG_COUNT" -gt 10 ]; then
    ...
    if [ ! -f "data/urls/urls.txt" ]; then
        ...
        python3 scripts/dataset/fetch_wikimedia_urls.py --out data/urls/urls.txt --count 300
    else
    ...
    python3 scripts/dataset/download_images.py data/urls/urls.txt datasets \
        --max 5000 \
        --split 0.8 \
        --delay 2.0 \
        --wikimedia-delay 3.0 \
...
python3 scripts/dataset/prepare_dataset.py
...
if [ -f "runs/detect/custom_run/weights/best.pt" ]; then
    ...
    if [ "$choice" == "2" ] || [ "$choice" == "" ]; then
        ...
        python3 scripts/train/train.py --max-epochs 50 --step 5 --target-map 0.60
    else
...
python3 scripts/utils/evaluate.py --weights runs/detect/custom_run/weights/best.pt --data config/data.yaml
